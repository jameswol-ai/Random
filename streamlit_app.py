...
st.markdown(
    """
<style>
@import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;600;700&family=Space+Grotesk:wght@400;700');
...
# ============================================================
# BIM SPACE GENERATOR (FIXED)
# ============================================================

def create_bim_plan(design):
    spaces = [
        {"name": "Central Living Atrium", "width": 7, "height": 5, "color": "#2563eb"},
        {"name": "AI Kitchen Hub", "width": 5, "height": 4, "color": "#059669"},
        {"name": "Service Core", "width": 3, "height": 3, "color": "#d97706"}
    ]
    for i in range(design["modules"]):
        spaces.append({
            "name": f"Adaptive Module {i+1}",
...
def project_summary(design):
    return {
        "Design ID": design["id"],
        "Building Type": design["building"],
        "Domain": design["domain"],
        "Area": area_display(design["area"]),
        "Modules": design["modules"],
        "Fitness": design.get("score", 0)
    }
...
<h1>📐 RANDOM AI BIM STUDIO V53</h1>
<p>Evolutionary Architecture • BIM Intelligence • Generative Spatial Design</p>
</div>
""",
    unsafe_allow_html=True
)
...
with st.sidebar:
    ...
    st.session_state.unit_system = st.selectbox(
        "Unit System", ["Metric", "Imperial", "Dual"]
    )
    building = st.selectbox(
        "Architectural Typology",
        sum(ARCHITECTURE_TYPES.values(), [])
    )
    modules = st.slider("Spatial Modules", 1, 10, 4)
    generations = st.slider("AI Evolution Cycles", 2, 30, 8)
...
with tabs[0]:
    ...
    c1.metric("Stored Designs", len(memory["designs"]))
    c2.metric("Evolution Runs", len(memory["evolution"]))
    c3.metric("System Version", "V53")
    st.divider()
    st.info("Generate a design in the Evolution Lab.")
...
with tabs[1]:
    st.subheader("🧬 AI Evolution Laboratory")

    if st.button("🚀 Generate BIM Design", type="primary"):
        with st.spinner("Evolving architectural DNA..."):
            ...
            memory["evolution"].append({
                "building": building,
                "modules": modules,
                "history": history
            })
            save_memory()
            log_event(f"Design {design['id']} evolved")

        st.success(f"Design {design['id']} generated!")
        st.json(project_summary(design))
        st.write("### Fitness Metrics")
        st.bar_chart(design["fitness"])
...
with tabs[2]:
    st.info("🏛️ BIM Viewer – coming soon")

with tabs[3]:
    st.info("🧱 Structural AI – coming soon")

with tabs[4]:
    st.info("💰 Cost + Materials – coming soon")

with tabs[5]:
    st.info("🌱 Sustainability – coming soon")

with tabs[6]:
    st.info("🧠 Memory Core – coming soon")
